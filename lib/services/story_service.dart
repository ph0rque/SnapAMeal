...
class StoryService {
  ...
  Future<void> postStory(
    ...
    try {
      Logger.d(
        "StoryService: Posting story - isVideo: $isVideo, filePath: $filePath",
      );
      ...
      if (isVideo) {
        Logger.d("StoryService: Processing video story...");

        // Validate video file
        if (!await VideoCompressionUtil.validateVideoFile(filePath)) {
          Logger.d("StoryService: Video file validation failed");
          return;
        }
        ...
        if (compressedFile == null) {
          Logger.d("StoryService: Video compression failed");
          return;
        }

        fileToUpload = compressedFile;
        Logger.d("StoryService: Video compressed successfully");
        ...
        if (thumbnailFile != null) {
          try {
            ...
            Logger.d("StoryService: Thumbnail uploaded successfully");

            // Clean up local thumbnail file
            await thumbnailFile.delete();
          } catch (e) {
            Logger.d("StoryService: Error uploading thumbnail: $e");
      ...
      Logger.d("StoryService: Story media uploaded successfully");
    } catch (e) {
      Logger.d("StoryService: Error uploading story media: $e");
      return;
    }
    ...
    try {
      await _firestore.collection('users').doc(user.uid).collection('stories').add({
        ...
        'permanence': {
          ...
          'expiresAt':
              FieldValue.serverTimestamp(), // Will be updated when first engagement happens
          'tier': 'standard',
      ...
      Logger.d("StoryService: Story document created successfully");
    } catch (e) {
      Logger.d("StoryService: Error creating story document: $e");
    }
  }
  ...
  Stream<List<DocumentSnapshot>> getFilteredStoriesForUserStream(
    String userId,
  ) async* {
    final now = DateTime.now();
    final twentyFourHoursAgo = now.subtract(const Duration(hours: 24));
    ...
    await for (final snapshot
      ...
      try {
        ...
        Logger.d('Error filtering stories: $e');
  ...
  Stream<List<Map<String, dynamic>>> getFilteredFriendsStoriesStream(
    ...
    await for (final _ in Stream.periodic(const Duration(seconds: 5))) {
      try {
          ...
          if (friendStories.docs.isNotEmpty) {
            ...
            if (filteredStories.isNotEmpty) {
              ...
              allStories.add({
                ...
                'filteredCount':
                    friendStories.docs.length - filteredStories.length,
              });
            }
          }
        }
        ...
        Logger.d('Error getting filtered friends stories: $e');
        yield [];
      }
    }
  }
  ...
  Future<void> updateStoryEngagement(
    String userId,
    String storyId,
    ...
    try {
      ...
      await storyRef.update({
        ...
        'totalEngagementScore': FieldValue.increment(
          _getEngagementWeight(engagementType),
        ),
      });
      ...
      Logger.d('Error updating story engagement: $e');
    }
  }
  ...
  Future<void> _calculateAndUpdateStoryPermanence(
    String userId,
    String storyId,
  ) async {
    try {
      ...
      Logger.d(
        'Updated story permanence: ${permanenceDuration.inHours} hours (tier: $permanenceTier)',
      );
    } catch (e) {
      Logger.d('Error calculating story permanence: $e');
    }
  }

  /// Calculate logarithmic duration based on engagement metrics
  Duration _calculateLogarithmicDuration(
    int totalScore,
    int views,
    int likes,
    int comments,
    ...
    final interactionQuality =
        (likes * 0.3) + (comments * 0.5) + (shares * 0.7);
  ...
  Future<List<DocumentSnapshot>> getMilestoneStories(
    String userId, {
    int limit = 20,
  }) async {
    try {
      ...
      Logger.d('Error getting milestone stories: $e');
      return [];
    }
  }
  ...
  Future<Map<String, dynamic>> getStoryAnalytics(
    ...
    try {
      ...
      Logger.d('Error getting story analytics: $e');
      return {};
    }
  }
